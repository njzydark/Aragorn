...
import { faToolbox, faUpload } from '@fortawesome/free-solid-svg-icons';
import './index.less';

export default function History() {
  const { uploadedFiles, uploaderProfiles } = useContext(AppContext);
  const isShowAddBtn = uploadedFiles.length === 0 && uploaderProfiles.length === 0;
...
function EmptyUploader() {
  ...
  return (
    <div className="empty-wrapper">
      <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无上传器配置">
        <div className="btn-group">
          <Button
            icon={<FontAwesomeIcon icon={faToolbox} style={{ marginRight: 10 }} />}
            type="primary"
            onClick={() => handleJump('uploaderProfile')}
          >
            添加上传器配置
          </Button>
        </div>
...
function HistoryList() {
  const { uploadedFiles, uploaderProfiles, setCurMenuKey } = useContext(AppContext);
  const history = useHistory();

  function getUploaderProfileName(id: string) {
    const uploader = uploaderProfiles.find(item => item.id === id);
  ...
  function handleUploaderProfileClick(id: string) {
    setCurMenuKey(id);
    history.push(`/uploaderProfile/${id}`);
  }
      ...
      {uploadedFiles.map(item => (
        <div className="list-item" key={item.name}>
          <div className="info">
            ...
            <div className="desc desc-uploader" onClick={() => handleUploaderProfileClick(item.uploaderProfileId)}>
              <span className="desc-title">上传方式：</span>
              <span className="desc-content">{getUploaderProfileName(item.uploaderProfileId)}</span>
            </div>
            <div className="desc">
